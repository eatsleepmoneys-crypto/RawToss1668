...
async function viewSlip(txId) {
  try {
    ...
    const res = await fetch(`${API_BASE}/admin/transactions/${txId}/slip`, {
      headers: { 'Authorization': 'Bearer ' + token }
    });
    if (!res.ok) {
      // Fallback: fetch transaction and use slip_image field directly
      const txRes = await fetch(`${API_BASE}/admin/transactions?id=${txId}`, {
        headers: { 'Authorization': 'Bearer ' + token }
      });
      ...
      if (tx && tx.slip_image) {
        ...
        win.document.write(`<html><body style="margin:0;background:#000;display:flex;align-items:center;justify-content:center;min-height:100vh"><img src="${tx.slip_image}" style="max-width:100%;max-height:100vh;object-fit:contain"></body></html>`);
        return;
      }
    ...
    alert('ไม่สามารถโหลดสลิปได้: ' + (e.message || e));
  }
}
...
async function renderDeposits(el) {
  ...
  el.innerHTML = `
    ...
    ${txs.length ? `<div class="card" style="padding:8px;overflow-x:auto">
      <table>
        <thead><tr>
          <th>REF</th><th>สมาชิก</th><th>จำนวน</th><th>วิธีชำระ</th><th>สลิป</th><th>วันที่</th><th style="min-width:160px">จัดการ</th>
        </tr></thead>
        <tbody>${txs.map(tx => `
          <tr>
            <td style="font-family:'JetBrains Mono',monospace;font-size:9px;color:#666">${(tx.uuid||'').slice(0,8)}</td>
            ...
            <td style="font-size:10px;color:#555">${new Date(tx.created_at).toLocaleDateString('th-TH',{hour:'2-digit',minute:'2-digit'})}</td>
            <td style="display:flex;gap:6px;padding:8px 4px;flex-wrap:wrap">
