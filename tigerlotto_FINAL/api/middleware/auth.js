...
const authAdmin = async (req, res, next) => {
  try {
    ...
    const [admin] = await query(
      'SELECT id, uuid, name, email, phone, balance, role, is_active FROM admins WHERE id = ? AND is_active = 1',
      [decoded.id]
    );
...
const authAgent = async (req, res, next) => {
  try {
    ...
    const [agent] = await query(
      'SELECT id, uuid, name, phone, email, commission_rate, referral_rate, balance, commission_balance, total_commission, bank_code, bank_account, bank_name, status FROM agents WHERE id = ? AND status = "active"',
      [decoded.id]
    );
