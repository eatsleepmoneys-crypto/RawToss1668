...
const authAgent = async (req, res, next) => {
  try {
    ...
    const [agent] = await query(
      'SELECT id, uuid, name, phone, email, commission_rate, referral_rate, balance, commission_balance, total_commission, status FROM agents WHERE id = ? AND status = "active"',
      [decoded.id]
    );
